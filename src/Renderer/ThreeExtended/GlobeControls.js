...
function GlobeControls(view, target, radius, options = {}) {
    ...
    var onMouseMove = (function getOnMouseMoveFn() {
        ...
        return function onMouseMove(event)
        {
            ...
            if (state === CONTROL_STATE.ORBIT || state === CONTROL_STATE.PANORAMIC) {
                if (state === CONTROL_STATE.ORBIT && !this.configurationUi.ORBIT.enable) return;
                if (state === CONTROL_STATE.PANORAMIC && !this.configurationUi.PANORAMIC.enable) return;
                ...
                if (this.configurationUi.DOLLY.enable === false) return;
                ...
                if (this.configurationUi.PAN.enable === false) return;
                ...
                if (this.configurationUi.MOVE_GLOBE.enable === false) return;
    ...
    this.configurationUi = {
        MOVE_GLOBE: {
            mouseButton: this.mouseButtons.LEFTCLICK,
            enable: true,
        },
        PANORAMIC: {
            mouseButton: this.mouseButtons.LEFTCLICK,
        ...
        DOLLY: {
            mouseButton: this.mouseButtons.MIDDLECLICK,
            enable: true,
        },
        PAN: {
            mouseButton: this.mouseButtons.RIGHTCLICK,
            enable: true,
        },
    };

    const UiToState = (mouseButton, keyboard) => {
        for (const key of Object.keys(this.configurationUi)) {
            const value = this.configurationUi[key];
            if (value.enable && value.mouseButton === mouseButton && value.keyboard === keyboard) {
    ...
    var onMouseWheel = function onMouseWheel(event) {
        player.stop().then(() => {
            if (!this.enabled || !this.configurationUi.DOLLY.enable) return;
    ...
    var onKeyUp = function onKeyUp() {
        if (this.enabled === false || this.enableKeys === false || this.configurationUi.PAN.enable === false) return;
    ...
    var onTouchStart = function onTouchStart(event) {
        ...
        switch (event.touches.length) {
            ...
            case 1: // one-fingered touch: rotate
                ...
                if (!this.configurationUi.ORBIT.enable) return;
            ...
            case 2: // two-fingered touch: dolly
                ...
                if (!this.configurationUi.DOLLY.enable) return;
...
GlobeControls.prototype.setCameraTargetPosition = function setCameraTargetPosition(position, isAnimated) {
    ...
    // this._view.onAfterRender = () => {
    //     const result = DEMUtils.getElevationValueAt(this._view.wgs84TileLayer, geo2Target, tile);
    //     geo2Target._values[2] = !result || result.z < 0 ? 0 : result.z;
    //     tile = result ? result.tile : undefined;

    //     const delta = startAltitude - geo2Target.altitude();
    //     console.log(delta);
    //     // if (position.range) {
    //     //     animatedScale = 1.0 - (delta + position.range) / startRange;
    //     // }
    ...
    if (isAnimated) {
        ...
        if (position.range) {
            animatedScale = 1.0 - position.range / startRange;
        }
...
GlobeControls.prototype.setCameraTargetGeoPosition = function setCameraTargetGeoPosition(coordinates, isAnimated) {
    isAnimated = isAnimated === undefined ? this.isAnimationEnabled() : isAnimated;
    const geoPosition = new C.EPSG_4326(coordinates.longitude, coordinates.latitude, 0);

    const result = DEMUtils.getElevationValueAt(this._view.wgs84TileLayer, geoPosition);
    geoPosition._values[2] = !result || result.z < 0 ? 0 : result.z;

    console.log(geoPosition._values[2], result);
    const position = geoPosition.as('EPSG:4978').xyz();
